...
describe('Integration Tests', () => {
  describe('CSV Import Flow', () => {
    it('should process CSV data end-to-end', async () => {
      ...
      assert.deepStrictEqual(sheetData[0], ['€ 50,00', 'John Doe', '2024-03-15']);
      assert.deepStrictEqual(sheetData[1], ['€ 25,00', 'Jane Smith', '2024-03-16']);
    });
  });
  ...
  describe('Email Sending Flow', () => {
    it('should filter users correctly for payment requests', () => {
      const users = [
        { 'E-mailadress': 'john@example.com', 'Totaal': '€ 50,00', 'Email Sent': '' },
        { 'E-mailadress': 'invalid', 'Totaal': '€ 30,00', 'Email Sent': '' },
        { 'E-mailadress': 'jane@example.com', 'Totaal': '€ 0,00', 'Email Sent': '' },
        { 'E-mailadress': 'bob@example.com', 'Totaal': '€ 20,00', 'Email Sent': '' }
      ];
    ...
    it('should filter users correctly for reminders', () => {
      ...
      const users = [
        { 'E-mailadress': 'john@example.com', 'Email Sent': '01-01-2024', 'Resterende': '€ 50,00' },
        { 'E-mailadress': 'jane@example.com', 'Email Sent': '05-01-2024', 'Resterende': '€ 30,00' },
        { 'E-mailadress': 'bob@example.com', 'Email Sent': '01-01-2024', 'Resterende': '€ 0,00' }
      ];

      const needsReminder = users.filter(user => {
        if (!user['Email Sent'] || user['Resterende'] === '€ 0,00') return false;
        const emailDate = new Date(user['Email Sent'].split('-').reverse().join('-'));
